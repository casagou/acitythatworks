...
ok(/HANNA, Christopher/.test(hub) && /Christopher Hanna/.test(hub), "Hanna missing from hub");
ok(/HANNA, Christopher/.test(roster) && /Christopher Hanna/.test(roster), "Hanna missing from roster");
ok(!fs.existsSync(path.join(ROOT, "profiles", "hanna.html")), "invented Hanna profile page exists");
ok((APPLIED.notScored || []).some((n) => n.name === "Christopher Hanna" && n.profile === false), "Hanna not in notScored as field-status-only");
ok(/Mayor, nomination accepted/.test(hubCard("cand-christopher-hanna")), "Hanna hub status missing nomination accepted");
ok(/Field status on the roster/.test(hubCard("cand-christopher-hanna")), "Hanna hub card is not field-status-only");
ok(!/Full profile/.test(hubCard("cand-christopher-hanna")), "Hanna hub card invented a full-profile link");
ok(!/expression of intent/i.test(hubCard("cand-christopher-hanna")), "Hanna hub has expression-of-intent copy");

function fieldStatusCouncil(id, cityLine, name, slug) {
  ok(hub.includes(cityLine) && hub.includes(name), name + " missing from hub");
  ok(roster.includes(cityLine) && roster.includes(name), name + " missing from roster");
  ok(!fs.existsSync(path.join(ROOT, "profiles", slug + ".html")), "invented " + slug + " profile page exists");
  ok((APPLIED.notScored || []).some((n) => n.name === name && n.profile === false), name + " not in notScored as field-status-only");
  ok(/Council candidate · nomination accepted/.test(hubCard(id)), name + " hub status missing nomination accepted");
  ok(/Field status on the roster/.test(hubCard(id)), name + " hub card is not field-status-only");
  ok(!/Full profile/.test(hubCard(id)), name + " hub card invented a full-profile link");
  ok(!/expression of intent/i.test(hubCard(id)), name + " hub has expression-of-intent copy");
}
fieldStatusCouncil("cand-herb-collins", "COLLINS, Herb", "Herb Collins", "collins");
fieldStatusCouncil("cand-steve-filipovic", "FILIPOVIC, Steve", "Steve Filipovic", "filipovic");
fieldStatusCouncil("cand-jason-heit", "HEIT, Jason", "Jason Heit", "heit");
fieldStatusCouncil("cand-james-harasymow", "HARASYMOW, James", "James Harasymow", "harasymow");
...
ok(/HARRIS, Mike/.test(atk) && /MCGUIGAN, Bruce/.test(atk) && /GERVAIS-HARRISON, Darren/.test(atk) && /HANNA, Christopher/.test(atk), "Atkinson page lost current accepted-mayor list");
...
/* City-accepted set: 8 mayor + 21 council. Name them; do not invent a count. */
const ACCEPTED_COUNCIL = [
  "Bowkett", "Caradonna", "Collins", "Cseszko", "Dell", "Dion",
  "Filipovic", "Garcia", "Gardiner", "Gibbs", "Girard", "Hammond",
  "Harasymow", "Heit", "Kim", "Lee", "Loughton", "McInnis", "Rothe", "Sandor", "Thompson"
];
const ACCEPTED_MAYORS = [
  "ALTO, Marianne", "ATKINSON, Lyall", "GERVAIS-HARRISON, Darren",
  "HANNA, Christopher", "HARRIS, Mike", "JOHNSTON, David", "MCGUIGAN, Bruce", "MILLER, Gregoor"
];
...
ok(/Bowkett, Caradonna, Collins, Cseszko, Dell, Dion, Filipovic, Garcia, Gardiner, Gibbs, Girard, Hammond, Harasymow, Heit, Kim, Lee, Loughton, McInnis, Rothe, Sandor and Thompson/.test(roster), "full accepted council list missing from roster");
ok(/ALTO, Marianne; ATKINSON, Lyall; GERVAIS-HARRISON, Darren; HANNA, Christopher; HARRIS, Mike; JOHNSTON, David; MCGUIGAN, Bruce; and MILLER, Gregoor/.test(roster), "full accepted mayor list missing from roster");
ok(!/ALTO, Marianne; ATKINSON, Lyall; GERVAIS-HARRISON, Darren; HARRIS, Mike; JOHNSTON, David; MCGUIGAN, Bruce; and MILLER, Gregoor/.test(rosterNote), "stale 7-mayor City list still on roster");
ok(!/Bowkett, Caradonna, Collins, Cseszko, Dell, Dion, Filipovic, Garcia, Gardiner, Gibbs, Girard, Hammond, Heit, Kim, Lee, Loughton, McInnis, Rothe, Sandor and Thompson/.test(rosterNote), "stale 20-council City list still on roster");
...
ok(!/Bowkett, Caradonna, Cseszko, Dell, Dion, Garcia, Gardiner, Gibbs, Girard, Hammond, Kim, Lee, Loughton, McInnis, Rothe, Sandor and Thompson/.test(rosterNote), "stale 17-council City list still on roster");
...
console.log("check-field-3sep: ok — letters/counts locked, City list 8 mayor + 21 council, TC attribution, Johnston/Miller/Gervais-Harrison/Hanna + Collins/Filipovic/Heit/Harasymow field-status only, /build 404 held");
